...
#100*sum(MPAs_merged$marine_area_km2, na.rm = T)/ocean_area # 8.16 %
...
Using the WDPA id and WPDA pid fields, let's merge the MPA guide data to the WDPA data to assign a protection level to each MPA in the WDPA database. Effectively, we'll need to replace MPA's in WDPA that have been assessed by MPA guide with the shapes from the MPA guide database.
...
We are able to assign a protection level to `r sum(!is.na(mWDPA_info$mpa_guide_lvls))` MPAs in the WDPA database, representing a total area of `r round(sum(mWDPA_info$marine_area_km2[!is.na(mWDPA_info$mpa_guide_lvls)], na.rm = T)/10^6)` million km2, `r round(100*sum(mWDPA_info$marine_area_km2[!is.na(mWDPA_info$mpa_guide_lvls)], na.rm = T)/ocean_area, 2)` % of the Ocean, and `r round(100*sum(mWDPA_info$marine_area_km2[!is.na(mWDPA_info$mpa_guide_lvls)], na.rm = T)/wdpa_totals$total_area_km2, 2)` % of the global marine protected area. The summary of these MPAs by protection level and establishment stage is shown below.
...
incompatible_mpa_guide |> 
  filter(! wdpa_id%in% unique(mWDPA_info$wdpa_id[mWDPA_info$mpa_guide_lvls == "incompatible"])) |> 
  arrange(desc(area_km2)) 
  mapview::mapview(zcol = c(area = "mpa_guide_lvl"), 
                 legend = TRUE,
                 map.types = c("Esri.WorldImagery"),
...
```{r}
mWDPA_scenario2 <- st_read(file.path(prj_path, "data/02_processed/mWDPA_scenario_2.gpkg"))
```
