...
territorial_waters <- st_read(file.path(prj_path, "data/02_processed/world_territorial_waters_v2.gpkg"))

#territorial_waters_v2 <- st_read(file.path(prj_path, "data/02_processed/world_territorial_waters_v2.gpkg"))
...
mWDPA_1_info <- mWDPA_1 |> 
  st_drop_geometry()

mWDPA_1_totals <- mWDPA_1 |> 
  st_drop_geometry() |> 
...
The final dataset contains `r mWDPA_1_totals$n_mpa` MPAs, covering a total of `r round(mWDPA_1_totals$protected_area_km2/10^6)` million km², which represents `r round(mWDPA_1_totals$pct_ocean,2)`% of the ocean. The median size of these MPAs is `r round(mWDPA_1_totals$median_size,2)` km², with an average size of `r round(mWDPA_1_totals$avg_size,2)` km².
...
mWDPA_1_info |> 
  ...
  # rename vars
  gt::cols_label(mpa_guide_lvls = "Protection Level",
                 n_mpa = "# of MPAs",
...
Almost 50% of the MPAs in the dataset are less than 1 km2 in size, with a global median size of `r round(mWDPA_1_totals$median_size,1)` km2.

```{r eval = T, include = T}
# Lets count the number of MPAs in different bin sizes (<1, 1-10, 10-100, 100-1000, 1000-10000, >10000)

mpas_by_size <- mWDPA_1_info |> 
  ...
  group_by(size_bin) |>
  summarize(n_mpa = n_distinct(wdpa_id),
            pct_mpas = 100*n_mpa/n_distinct(mWDPA_1_info$wdpa_id), 
            median_size = median(marine_area_km2),
            avg_size = mean(marine_area_km2),
            std_size = sd(marine_area_km2),
            protected_area_km2 = sum(marine_area_km2),
...
labels_count <- mWDPA_1_info |> 
  ...
  ungroup() |> 
  mutate_if(is.numeric, round, 2) |> 
                     mutate(label = paste0(mpa_guide_lvls, " (", n_mpa, ")")) |> 
                     select(mpa_guide_lvls, label) |> 
                     deframe()

(size_boxplot <- mWDPA_1_info |> 
  ggplot(aes(x = mpa_guide_lvls, y = marine_area_km2, col = mpa_guide_lvls, fill = mpa_guide_lvls))+
...
ggsave(file.path(prj_path, "figures/size_boxplot.pdf"), size_boxplot, width = 12, height = 10)
ggsave(file.path(prj_path, "figures/size_boxplot.png"), size_boxplot, width = 12, height = 10)
```

```{r eval = T}
mWDPA_1_within_12nm <- st_read(file.path(prj_path, "data/02_processed/mWDPA_s1_within_12nm.gpkg"))

mWDPA_1 <- mWDPA_1 |> 
  left_join(st_drop_geometry(mWDPA_1_within_12nm)) |> 
  replace_na(list(area_in_12nm_km2 = 0)) |>
  mutate(pct_area_in_12nm = 100*area_in_12nm_km2/marine_area_km2)

within_12nm <- st_drop_geometry(mWDPA_1)  |> 
  filter(marine_area_km2 >= 0.001) |> 
  mutate(in_12_nm = if_else(pct_area_in_12nm >= 90, "< 12nm", "12 - 200nm") ) |> 
  group_by(in_12_nm) |> 
  summarize(n_mpas = n_distinct(wdpa_id),
            proportion = n_mpas/n_distinct(mWDPA_1$wdpa_id),
...
We classified MPAs as inside territorial waters if at least 90% of its total area falls within 12nm from the coast. Else, if territorial waters represented less than 10% of the MPA area, it was considered as outside (12-200nm). A total of `r within_12nm$n_mpas[within_12nm$in_12_nm == T]` MPAs (`r round(100*within_12nm$n_mpas[within_12nm$in_12_nm == T]/mWDPA_1_totals$n_mpa)` % of total) are fall within the territorial seas of countries yet they represent only `r within_12nm$pct_ocean[within_12nm$in_12_nm == T]`% of the ocean. The median size of these MPAs is `r within_12nm$median_size_km2[within_12nm$in_12_nm == T]` km². On the other hand, MPAs outside territorial waters represent `r within_12nm$pct_ocean[within_12nm$in_12_nm == F]`% of the ocean, with a median size of `r round(within_12nm$median_size_km2[within_12nm$in_12_nm == F],2)` km².

```{r}
(size_by_location <- st_drop_geometry(mWDPA_1)  |> 
  filter(marine_area_km2 >= 0.001) |> 
  mutate(in_12_nm = if_else(pct_area_in_12nm >= 90, "< 12nm", "12 - 200nm") ) |> 
  ggplot(aes(x = in_12_nm, y = marine_area_km2, col = in_12_nm, fill = in_12_nm))+
  geom_boxplot(show.legend = F, width = .2, fill = "white", size = 1, outlier.shape = NA)+
  scale_y_continuous(transform = "log10", breaks = c(0,.1,1,10,100,1000,100000))+
  ggdist::stat_halfeye(adjust = .33, ## bandwidth
                       width = .67, 
...
ggsave(file.path(prj_path, "figures/S1_MPA_size_by_location.pdf"), size_by_location, width = 12, height = 10)  
ggsave(file.path(prj_path, "figures/S1_MPA_size_by_location.png"), size_by_location, width = 12, height = 10)  
```
...
```{r eval = F}
protected_poly_s1 <- st_read(file.path(prj_path, "data/02_processed/s1_protected_poly.gpkg"))
...
write_csv(results_by_iso_s1, file.path(prj_path, "data/03_output/S1_results_by_iso.csv"))
...
world_eez |> 
  ...
  st_write(file.path(prj_path, "data/03_output/S1_world_eez_with_results.gpkg"), 
           append = F)
...
territorial_waters |> 
  ...
  st_write(file.path(prj_path, "data/03_output/S1_territorial_waters_with_results.gpkg"), append = F)
...
eez_by_iso |> 
  ...
  st_write(file.path(prj_path, "data/03_output/S1_sov_eez_with_results.gpkg"), append = F)
```

To date, 12 sovereign nations have reached or exceed the 30% protection goals. These are:

```{r eval = T, include = T}
results_by_iso_s1 <- read_csv(file.path(prj_path, "data/03_output/S1_results_by_iso.csv"))
...
ggsave(file.path(prj_path, "figures/S1_scatter_by_iso.png"), scatter_by_iso, width = 12, height = 10)
ggsave(file.path(prj_path, "figures/S1_scatter_by_iso.pdf"), scatter_by_iso, width = 12, height = 10)
```
...
```{r eval = F}
protected_poly_s2 <- st_read(file.path(prj_path, "data/02_processed/s2_protected_poly.gpkg"))
...
write_csv(results_by_iso_s2, file.path(prj_path, "data/03_output/S2_results_by_iso.csv"))
...
world_eez |> 
  left_join(results_by_mrgid_s2 |> 
            ...
            by = "mrgid") |> 
  mutate(pct_eez_protected = round(100*area_mpa_km2/area_km2, 2)) |> 
  st_write(file.path(prj_path, "data/03_output/S2_world_eez_with_results.gpkg"), 
           append = F)
...
territorial_waters |> 
  left_join(results_by_mrgid_s2 |> 
            ...
            by = "mrgid") |> 
  mutate(pct_12nm_protected = round(100*area_12nm_mpa_km2/area_12nm_km2, 2)) |> 
  st_write(file.path(prj_path, "data/03_output/S2_territorial_waters_with_results.gpkg"), append = F)
...
eez_by_iso |> 
  ...
  st_write(file.path(prj_path, "data/03_output/S2_sov_eez_with_results.gpkg"), append = F)
```

### Most effective protection

```{r eval = F}
protected_poly_s3 <- st_read(file.path(prj_path, "data/02_processed/s3_protected_poly.gpkg"))
...
write_csv(results_by_iso_s3, file.path(prj_path, "data/03_output/s3_results_by_iso.csv"))
...
world_eez |> 
  left_join(results_by_mrgid_s3 |> 
            ...
            by = "mrgid") |> 
  mutate(pct_eez_protected = round(100*area_mpa_km2/area_km2, 2)) |> 
  st_write(file.path(prj_path, "data/03_output/s3_world_eez_with_results.gpkg"), 
           append = F)
...
territorial_waters |> 
  left_join(results_by_mrgid_s3 |> 
            ...
            by = "mrgid") |> 
  mutate(pct_12nm_protected = round(100*area_12nm_mpa_km2/area_12nm_km2, 2)) |> 
  st_write(file.path(prj_path, "data/03_output/s3_territorial_waters_with_results.gpkg"), append = F)
...
eez_by_iso |> 
  ...
  st_write(file.path(prj_path, "data/03_output/s3_sov_eez_with_results.gpkg"), append = F)
```
